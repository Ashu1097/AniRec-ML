...
def build_scorer_from_checkpoint(
    ...
    model = AniRecV20(n_u, n_i, n_g, use_text=has_text, use_tone=has_tone).to(_device)
    ...
    if os.path.exists(checkpoint_path):
        ...
        try:
            ...
            sd = OrderedDict({k.replace("_orig_mod.", ""): v for k, v in ck["model"].items()})
            model.load_state_dict(sd, strict=False)
        logger.info("Loaded checkpoint: %s", checkpoint_path)
    else:
        logger.warning("Checkpoint not found at %s — using untrained model.", checkpoint_path)

    item_content_cpu = torch.FloatTensor(data["item_content"])
    seqs_cpu = torch.LongTensor(data["seqs"])
    item_text_cpu = torch.FloatTensor(data["item_text_embeddings"]) if has_text else None
    ...
    return InferenceScorer(
        ...
        item_content_cpu=item_content_cpu,
        seqs_cpu=seqs_cpu,
        ...
        anime_names=data["anime_names"],
        n_anime=data["n_anime"],
        item_domain=data["item_domain"],
...
class InferenceScorer:
    ...
    def __init__(
        ...
        item_domain,
        feedback_store=None,
        item_recency_norm=None,
        item_year=None,
        ani2i=None,
        item_sequel_ids=None,
        top_k_cands=2000,
        pop_floor=0.0,
        score_weights=None,
        use_amp=True,
        ...
        self.item_text_np = item_text_cpu.numpy() if item_text_cpu is not None else None
    ...
    def find_item(self, query: str):
        ...
        if self._smart_search is None:
            self._smart_search = SmartSearch(self.anime_names, self.movie_names, self.item_text_np)
        return self._smart_search.find(query)

    def recommend_from_items(self, item_ids, k=10, user_id=None, ui_context="all", epsilon=0.0):
        scores = []
        ...
        results = [self.anime_catalog[item_id] for item_id, _ in top_items]

        return results, top_items

    def recommend_by_names(self, names, k=10, user_id=None, ui_context="all", epsilon=0.0):
        ids = []
        for nm in names:
        ...
        return self.recommend_from_items(
            ids, k=k, user_id=user_id, ui_context=ui_context, epsilon=epsilon
        )

    def recommend_from_watchlist(self, username, watchlist=None, k=10, ui_context="all"):
        raise NotImplementedError("Full watchlist scoring is in notebooks/experimentation.ipynb.")
