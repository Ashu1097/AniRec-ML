...
def _encode_genres_list(items: List[dict],
                        g2i: Dict[str, int]) -> np.ndarray:
...
def load_dataset_v19(
    ...
    item_domain = {**{i: 0 for i in range(n_ani)},
                   **{i: 1 for i in range(n_ani, n_i)}}
    ...
    for uname, records in anime_interactions.items():
        ...
        for rec in records:
            ...
            if idx is None or weight < MIN_WEIGHT_THRESHOLD:
                continue
            tr_u.append(uid); tr_i.append(idx)
            tr_d.append(0);   tr_w.append(weight); tr_t.append(ts)
    ...
    for uname, mlist in movie_interactions.items():
        ...
        for entry in mlist:
            if not isinstance(entry, dict):
                continue
            mid = (entry.get("imdb_num") or entry.get("item_id") or
                   entry.get("imdb_id"))
            ...
            if weight < MIN_WEIGHT_THRESHOLD:
                continue
            tr_u.append(uid); tr_i.append(mov2i[int(mid)])
            tr_d.append(1);   tr_w.append(weight); tr_t.append(ts)
    ...
    if len(tr_u) > 0:
        ...
        tr_u, tr_i, tr_d, tr_w, tr_t = (
            tr_u[mask], tr_i[mask], tr_d[mask], tr_w[mask], tr_t[mask])
    ...
    for u, i, d, w, t in zip(tr_u, tr_i, tr_d, tr_w, tr_t):
        user_hist.setdefault(int(u), []).append(
            (int(i), int(d), float(w), int(t)))
    ...
    for uid, hist in user_hist.items():
        ...
        vi, vd, _, _ = hist[-2]; val_data.append((uid, vi, vd))
        ti, td, _, _ = hist[-1]; test_data.append((uid, ti, td))

    train_arr = (np.array([(t[0], t[1], t[2]) for t in train_triples],
                           dtype=np.int32)
                 if train_triples else np.zeros((0, 3), dtype=np.int32))
    train_w = (np.array([t[3] for t in train_triples], dtype=np.float32)
               if train_triples else np.zeros(0, dtype=np.float32))
    val_arr  = (np.array(val_data,  dtype=np.int32)
                if val_data  else np.zeros((0, 3), dtype=np.int32))
    test_arr = (np.array(test_data, dtype=np.int32)
                if test_data else np.zeros((0, 3), dtype=np.int32))
    ...
    anime_train_idx = (np.where(tr_d2 == 0)[0].astype(np.int32)
                       if len(tr_d2) else np.zeros(0, dtype=np.int32))
    movie_train_idx = (np.where(tr_d2 == 1)[0].astype(np.int32)
                       if len(tr_d2) else np.zeros(0, dtype=np.int32))

    # ── Content ───────────────────────────────────────────────────────────
    all_items_meta = ([anime_catalog[a] for a in anime_list] +
                      [movie_catalog[m] for m in movie_list])
    ...
    user_seq_snapshot = {uid: [h[0] for h in hist][-max_seq_len:]
                         for uid, hist in user_hist.items()}
    ...
    A_tilde.sort_indices(); A_tilde.eliminate_zeros()
    del A, di, deg; gc.collect()
    ...
    for a in anime_list:
        ...
        if item_pop_raw[ai] < 1.0:
            item_pop_raw[ai] = max(
                float(anime_catalog[a].get("popularity") or 0) / 1000.0, 1.0)
    for m in movie_list:
        mi = mov2i[m]
    ...
    for a in anime_list:
        ...
        try: item_year[ani2i[a]] = int(yr_str)
        except ValueError: item_year[ani2i[a]] = 0
    for m in movie_list:
        yr_str = str(movie_catalog[m].get("release_year", "") or "")[:4]
        try: item_year[mov2i[m]] = int(yr_str)
        except ValueError: item_year[mov2i[m]] = 0
    ...
    # Names
    anime_names = {ani2i[a]: anime_catalog[a].get("title", f"anime_{a}")
                   for a in anime_list}
    movie_names = {mov2i[m]: movie_catalog[m].get("title", f"movie_{m}")
                   for m in movie_list}
    item_genres = {ani2i[a]: anime_catalog[a].get("genres", [])
                   for a in anime_list}
    item_genres.update({mov2i[m]: movie_catalog[m].get("genres", [])
                        for m in movie_list})

    return dict(
        n_users=n_u, n_items=n_i, n_anime=n_ani, n_movies=n_mov,
        n_genres=len(g2i),
        ...
        ani2i=ani2i, mov2i=mov2i, al2i=al2i,
