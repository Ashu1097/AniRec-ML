...
class ProgressBar:
    ...
    def __init__(self, total: int, desc: str = "", unit: str = "it", colour: str = "cyan") -> None:
    ...
    def _render(self) -> None:
        ...
        line = (
            ...
            f"  eta {self._fmt_eta(eta)}" + (f"  {pfix}" if pfix else "")
        )
    ...
    def update(self, n: int = 1, **postfix: Any) -> None:
        ...
        if self._bar:
            if postfix:
                self._bar.set_postfix(**{k: str(v) for k, v in self._postfix.items()})
            self._bar.update(n)
