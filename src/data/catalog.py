...
def load_cached_data():
    ...
    if ANIME_CATALOG_PATH.exists():
        ...
        anime_catalog = {
            int(item["id"]): item for item in data if isinstance(item, dict) and "id" in item
        }

    if MOVIE_CATALOG_PATH.exists():
        with open(MOVIE_CATALOG_PATH, encoding="utf-8") as f:
            data = json.load(f)
    ...
    if INTERACTIONS_PATH.exists():
        with open(INTERACTIONS_PATH, encoding="utf-8") as f:
            saved = json.load(f)
        interaction_source = saved.get("anime", saved) if isinstance(saved, dict) else {}
    ...
    if ML_INTERACTIONS_PATH.exists():
        with open(ML_INTERACTIONS_PATH, encoding="utf-8") as f:
